...
<nav class="navbar navbar-light" style="background-color: #55468c !important;">
  <a class="navbar-brand" href="#">
   <strong style="color: #fff">TraviDev</strong>
  </a>
</nav>
...
foreach ($_REQUEST['correo'] as $key => $emailCliente ) {
    ...
    $queryViewUser= ("SELECT * FROM clients WHERE correo='".$emailCliente."'");
    $resultViewUser = mysqli_query($con,$queryViewUser );
    while ($dataUser = mysqli_fetch_array($resultViewUser)) {

$destinatario = trim($emailCliente); //Quitamos algun espacion en blanco
$asunto       = "Entrega de cerificado del evento talll ";
$cuerpo = '
    <!DOCTYPE html>
    <html lang="es">
    <head>
...
<body>
    ...
    <tr>
        <td style="background-color: #ffffff;">
            <div class="misection">
                <h2 style="color: red; margin: 0 0 7px">Hola, '.$dataUser['cliente'].'</h2>
                <p style="margin: 2px; font-size: 18px">'.$dataUser['mail'].' </p>
            </div>
        </td>
    </tr>
    <tr>
...
$cuerpo .= '
    ...
    $headers .= 'From: SoporteAdministración <soporteadministracion@undc.edu.pe>' . "\r\n";
    $headers .= 'Cc: noresponder@undc.edu.pe' . "\r\n";
